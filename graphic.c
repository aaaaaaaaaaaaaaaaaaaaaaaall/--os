...
if(cursor[y][x] =='a'){
                mouse[y * 16 + x] = COL8_FF0000;
        }
if(cursor[y][x] =='k'){
                mouse[y * 16 + x] = COL8_FFFF00;
}
if(cursor[y][x] =='t'){
                mouse[y * 16 + x] = COL8_848400;
}
}
...
void init_mouse_cursor090(char *mouse, char bc){
    int x;
    int y;
    ...
    if(task->langmode == 1){
        while(*s != '\0'){
            if(task->langbyte1 == 0){
                ...
                if(task->langbyte1 >= 0x81 && task->langbyte1 <= 0x9f){
...
void init_screen020(char *vram, int xsize, int ysize){
