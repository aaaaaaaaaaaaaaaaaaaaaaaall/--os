...
init_screen020(buf_back, binfo->scrnx, binfo->scrny);
    //mouse

    sht_mouse = sheet_alloc(shtctl);
    sheet_setbuf(sht_mouse, buf_mouse, 16, 16, 99);
   init_mouse_cursor090(buf_mouse, 99);
    mx = (binfo->scrnx - 16) / 2;
   ...
   //ythdrxxxxxxxxxfgggggtydhtyhtrhtrhdrt
        ...
        if(fifo32_status(&fifo) == 0){
            ...
            //マウス
                ...
                if(mouse_decode(&mdec, i - 512) != 0){
                    ...
                    //画面外に行かないようにする
                    if(mx < 0) mx = binfo->scrnx - 1;
                    if(my < 0) my = binfo->scrny - 1;
                    if(mx > binfo->scrnx - 1) mx = 0;
                    if(my > binfo->scrny - 1) my = 0;
