        ...
        if(fifo32_status(&task->fifo) == 0){
            ...
            if(i <= 1){
                if(i != 0){
                    ...
                    if(cons.cur_c >= 0 && no == 1){
                        cons.cur_c = COL8_00FF00;
                    }
 if(cons.cur_c >= 0 && no == 0){
            ...
            if(i == 2 && no==1){ //cursor on
                cons.cur_c = COL8_00FF00;
            }
            if(i == 2 && no==0){ //cursor on n=0
                cons.cur_c = COL8_FFFFFF;
            }
            if(i == 3){ //cursor off
                if(cons.sht != 0){
            ...
            if(i >= 256 && i <= 511 && no==1){
                if(i == 8 + 256){ //backspace
                    ...
                    //プロンプト表示
                   cons_putstr0(&cons, "  n o  ba m a n >");
                }
                else{
                    if(cons.cur_x < 240){
                        cmdline[cons.cur_x / 8 - 18] = i - 256;
...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, unsigned int memtotal){
   ...
   else if(_strcmp(cmdline, "nobaman") == 0 && cons->sht != 0){
      no = 1;
  ...
  else if(_strcmp(cmdline, "oneshot") == 0 && cons->sht != 0&& no == 1){
	cons_putstr0(cons, "majidekamige yattahougaii\n\n");
    }
    else if(cmdline[0] != 0){
